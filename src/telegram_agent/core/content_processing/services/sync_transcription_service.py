...
class SyncTranscriptionService:
    ...
    def execute(self, *, job_id: UUID, retry_count: int) -> StageExecutionResult:
        try:
            ...
            result = WhisperXClient(self._settings).transcribe(
    ...
    def _retry_or_fail(self, *, job_id: UUID, retry_count: int, error_message: str) -> StageExecutionResult:
        if retry_count >= self._settings.media_task_max_retries:
            self._mark_failed(job_id, "Transcription retry limit exhausted")
            return StageExecutionResult(error_message="Transcription retry limit exhausted")
    ...
    def _claim_and_resolve(self, job_id: UUID) -> TranscriptionContext | None:
        with self._uow_factory() as uow:
            ...
            if asset is None or asset.media_type not in _TRANSCRIBABLE_ATTACHMENT_TYPES:
                uow.jobs.mark_failed(job_id=job_id, error_message="Media type is not transcribable")
                return None
            if not asset.local_path:
                uow.jobs.mark_failed(job_id=job_id, error_message="Downloaded media file is missing")
                return None
        ...
        *,
        ...
        with self._uow_factory() as uow:
            if not uow.transcripts.record(command):
                raise RetryableContentProcessingError("Transcript result could not be persisted")
            if not uow.jobs.complete_transcription(job_id=context.job_id):
                raise RetryableContentProcessingError("Transcription result could not be applied to job state")

    def _mark_retryable(self, job_id: UUID, error_message: str) -> None:
        with self._uow_factory() as uow:
            uow.jobs.mark_transcription_retryable(job_id=job_id, error_message=error_message)
