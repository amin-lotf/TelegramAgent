...
from telegram_agent.core.telegram_ingress.api.v1.router import api_router
...
def create_app() -> FastAPI:
    ...
    f_app = FastAPI(
        title="telegram orchestrator API",
        description="telegram orchestrator API for FatolAI WebAnalyzer",
        version="0.1.0",
        lifespan=lifespan,
    )
    ...
    @f_app.get("/api/",)
    async def root():
        return {"status": "ok", "service": "telegram orchestrator", "version": "0.1.0"}
