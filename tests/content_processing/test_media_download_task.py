...
def test_duplicate_media_download_task_execution_claims_job_once(
    ...
    monkeypatch.setattr(media_download, "sync_content_processing_uow_factory", content_sync_uow_factory)

    media_download.download_telegram_source_task.run(str(job_id))
    media_download.download_telegram_source_task.run(str(job_id))
