...
from telegram_agent.core.llm_gateway.common.schemas import GlossaryExtractionResponse
...
@lru_cache(maxsize=1)
def _build_glossary_extraction_service() -> GenerationService:
    if settings.openai_api_key is None:
        raise LlmGatewayAuthenticationError("LLM provider is not configured")
    structured = get_operator().with_structured_output(GlossaryExtractionResponse)
    return GenerationService(llm=structured, provider_name="openai")


async def get_glossary_extraction_service() -> GenerationService:
    return _build_glossary_extraction_service()
