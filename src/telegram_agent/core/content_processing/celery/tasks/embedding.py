...
@celery_app.task(name="media.embed", bind=True)
def embed_media_task(self, job_id: str) -> None:
    try:
        parsed_job_id = UUID(job_id)
    except (TypeError, ValueError):
        logger.warning(
            "Ignoring embedding task with invalid job id",
            extra={"job_id": job_id},
        )
        return

    result = SyncEmbeddingService.from_settings().execute(
