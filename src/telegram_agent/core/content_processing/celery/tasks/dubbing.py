...
@celery_app.task(name="dubbing.advance", bind=True)
def advance_dubbing_task(self, job_id: str) -> None:
    try:
        parsed_job_id = UUID(job_id)
    except (TypeError, ValueError):
        logger.warning("Ignoring dubbing task with invalid job id", extra={"job_id": job_id})
        return
    result = SyncDubbingWorkflowService.from_settings().execute(
        job_id=parsed_job_id, retry_count=self.request.retries
    )
    if result.deferred:
        advance_dubbing_task.apply_async(
            args=(job_id,),
