...
@celery_app.task(name="download.deliver", bind=True)
def deliver_download_task(self, job_id: str) -> None:
    try:
        parsed_job_id = UUID(job_id)
    except (TypeError, ValueError):
        logger.warning(
            "Ignoring download delivery task with invalid job id",
            extra={"job_id": job_id},
        )
        return

    result = SyncDownloadDeliveryService.from_settings().execute(
